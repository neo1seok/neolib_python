...
def crypto(msg,key,mode=SM4_ENCRYPT):
	clear_num = int.from_bytes(msg,'big')
	mk = int.from_bytes(key, 'big')
	crypto_num = _crypt(num=clear_num, mk=mk, mode=mode)
	return crypto_num.to_bytes(16,'big')
...
def decrypt(cipher_num, mk):
	...
	:param mk: 密钥, 16byte
	"""
	return crypto(cipher_num, mk,mode=SM4_DECRYPT)


class BlockCipherSM4(BaseBlockCipher128):
	def encrypt_round(self, Src):
		return encrypt( Src,self.user_key)


	def decrypt_round(self, Src):
		return decrypt(Src,self.user_key)

if __name__ == '__main__':
	from neolib.hexstr_util import  *
	tv_plain =	bytes([0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
	...
	tv_cipher =bytes([0x68, 0x1e, 0xdf, 0x34, 0xd2, 0x06, 0x96, 0x5e,
	 0x86, 0xb3, 0xe9, 0x4f, 0x53, 0x6e, 0x42, 0x46])


	tv_key =bytes([0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
	 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10])



	cipher = encrypt(clear_num=tv_plain ,mk=tv_key)

	#
	#

	print(tohexstr(cipher),tohexstr(tv_cipher))
	exit()
