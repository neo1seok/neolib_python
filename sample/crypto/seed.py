...
class SEED :
	...
	def ECB_dec(self,src):
		return self.crypto_template(src, None, lambda src, struct: src, lambda dst,struct: dst, self.SeedDecrypt)
		#return self.ECB_calc(src, self.SeedDecrypt)
		#None

	def ECB_enc(self,src):
		return self.crypto_template(src, None, lambda src, struct: src, lambda dst,struct: dst, self.SeedEncrypt)
		#return self.ECB_calc(src,self.SeedEncrypt)
	...
	def CBC_enc(self,src,iv):
		...
		return self.crypto_template(src, iv,prev_iv_process , post_iv_process, self.SeedEncrypt)
...
#			cypherkey = bytes([dst[idx] ^ struct.iv[idx] for idx in range(len(dst))])
		...
		return self.crypto_template(src, iv, prev_iv_process, post_iv_process, self.SeedDecrypt)
	...
	def CTR_enc(self,src,nonce):
		return self.CTR_Comm(src,nonce,self.SeedEncrypt)

	def CTR_dec(self, src, nonce):
		return self.CTR_Comm(src, nonce, self.SeedEncrypt)
...
if __name__ == '__main__' :
	...
	#plan =  neoutil.HexString2ByteArray('000102030405060708090A0B0C0D0E0Faabbcc'+'00'*13)
	plan = neoutil.HexString2ByteArray('000102030405060708090A0B0C0D0E0Faabbcc')
	#plan = neoutil.HexString2ByteArray('aabbcc' + '00' * 13)
	ciph = neoutil.HexString2ByteArray('5EBAC6E0054E166819AFF1CC6D346CDB')
	...
	#
	# enc = s.CTR_enc(plan, b'\x00' * 16)
	# dec = s.CTR_dec(enc, b'\x00' * 16)
	#
	# print(neoutil.ByteArray2HexString(enc))
	# print('90689FE77C9676197D181360EF1FA19D02B24A4319CBBE1AF28FDFB20B22C6D6')
